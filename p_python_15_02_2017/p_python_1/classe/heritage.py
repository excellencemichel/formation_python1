 ...
 classe mère
    ...
    -Une classe hérite d'une classe:
       -si la classe fille à un constructeur, c'est bien-sûr le contructeur de
        ...
        compte certaint att du const de la classe mèr, la classe fille n'aura pas accès
        à ces attributs qui sont définit dans la classe mère qui n'ont été pris en compte dans
        le contructeur de la classe fille, pour palier on peut appeller la classe mèr dans le const de la
        classe fille pour que quand on instantie la classe fille , le const de la classe fille
        pass par la classe mèr pour appeller inplicitement le contruct de la c mèr et
        cela permet de mettre à disposition de la classe fille l'essemble des att
        de la classe mèr,
        donc les att en commun avec la classe mèr sont juste passés en parametre du
        constructeur de la classe fille et donné en argument de l'appel de la classe mèr
        sans pour autant redéfir ces att dans le corps du construt de la classe fil
    ...
    d'une classe mère on interroge la fonction:
       'issubclass', qui donne True si vrai , False sinon
       exp : class A:
             class B(A)
             issubclass(B, A) =True,
...
class Uni:
    """
     Classe Uni, pour illustrer l'exemple d'héritage
...
class Fac(Uni):
    """
     ...
     Elle reprend les mêmes méthodes et att de la classe Uni
    """
